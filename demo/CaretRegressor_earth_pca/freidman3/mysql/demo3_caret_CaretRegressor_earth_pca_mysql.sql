...
FROM `ADS_pre_1_OUT`), 
earth_model_cte AS 
(SELECT earth_input.`KEY` AS `KEY`, -0.8104395754511003 * greatest(-1.132035998277834 - earth_input.`PC3`, 0) + 0.1771238026417063 * greatest(-0.5844284649024422 - earth_input.`PC2`, 0) + -0.1618641012977912 * greatest(0.4732590758099291 - earth_input.`PC1`, 0) + -0.1103024330973334 * greatest(earth_input.`PC1` - -1.3929600097741792, 0) + 0.1227648108261379 * greatest(earth_input.`PC3` - -0.9903186495144072, 0) + 1.508249314352372 AS `Estimator` 
FROM earth_input)
