...
FROM `ADS_sca_1_OUT`), 
`DT_node_lookup` AS 
(SELECT `ADS_pre_1_OUT`.`KEY` AS `KEY`, CASE WHEN (`ADS_pre_1_OUT`.`PC1` >= -0.6346714418532102) THEN CASE WHEN (`ADS_pre_1_OUT`.`PC4` < 0.6686887521268174) THEN 4 ELSE 5 END ELSE 3 END AS node_id_2 
FROM `ADS_pre_1_OUT`), 
`DT_node_data` AS 
(SELECT `Values`.nid AS nid, `Values`.`E` AS `E` 
FROM (SELECT 3 AS nid, 203.35 AS `E` UNION ALL SELECT 4 AS nid, 104.8805031446541 AS `E` UNION ALL SELECT 5 AS nid, 171.16666666666669 AS `E`) AS `Values`), 
`DT_Output` AS 
