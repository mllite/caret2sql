...
FROM "ADS_sca_1_OUT"), 
"DT_node_lookup" AS 
(SELECT "ADS_pre_1_OUT"."KEY" AS "KEY", CASE WHEN ("ADS_pre_1_OUT"."PC5" < -0.922348619) THEN 1 ELSE CASE WHEN ("ADS_pre_1_OUT"."PC3" < -0.701141179) THEN 3 ELSE 4 END END AS node_id_2 
FROM "ADS_pre_1_OUT"), 
"DT_node_data" AS 
(SELECT "Values".nid AS nid, "Values"."Estimator" AS "Estimator" 
FROM (SELECT 1 AS nid, -84.7875519 AS "Estimator" UNION ALL SELECT 3 AS nid, -30.5873508 AS "Estimator" UNION ALL SELECT 4 AS nid, 60.2983093 AS "Estimator") AS "Values"), 
"DT_Output" AS 
...
FROM "DT_Output"), 
"DT_node_lookup_1" AS 
(SELECT "ADS_pre_1_OUT"."KEY" AS "KEY", CASE WHEN ("ADS_pre_1_OUT"."PC7" < -0.0698422939) THEN CASE WHEN ("ADS_pre_1_OUT"."PC5" < 0.584405065) THEN 3 ELSE 4 END ELSE CASE WHEN ("ADS_pre_1_OUT"."PC5" < -0.40604803) THEN 5 ELSE 6 END END AS node_id_2 
FROM "ADS_pre_1_OUT"), 
"DT_node_data_1" AS 
(SELECT "Values".nid AS nid, "Values"."Estimator" AS "Estimator" 
FROM (SELECT 3 AS nid, -47.7487259 AS "Estimator" UNION ALL SELECT 4 AS nid, 29.7972469 AS "Estimator" UNION ALL SELECT 5 AS nid, -14.5889893 AS "Estimator" UNION ALL SELECT 6 AS nid, 54.3916435 AS "Estimator") AS "Values"), 
"DT_Output_1" AS 
