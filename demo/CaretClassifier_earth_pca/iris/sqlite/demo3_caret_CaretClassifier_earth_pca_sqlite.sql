...
FROM "ADS_pre_1_OUT"), 
earth_model_cte AS 
(SELECT earth_input."KEY" AS "KEY", 0.5652542518760738 * max(-0.017360589210772742 - earth_input."PC1", 0) + -2.085242044132094 * max(earth_input."PC3" - 0.2280435636734822, 0) + -3.39347876518004 * max(0.2280435636734822 - earth_input."PC3", 0) + -31.105006767610195 * max(earth_input."PC1" - 1.9193410513160611, 0) + 1.494746705263822 * max(earth_input."PC4" - 0.1047646025983524, 0) + 246.5306887644564 * max(earth_input."PC1" - -0.674082655111522, 0) + -211.2310381967572 * max(earth_input."PC1" - -0.8617563505066398, 0) + -26.468840824983282 AS "Score_0", -8.992786819418152 * max(-0.017360589210772742 - earth_input."PC1", 0) + 15.849955443066829 * max(earth_input."PC3" - 0.2280435636734822, 0) + -230.48053812141384 * max(0.2280435636734822 - earth_input."PC3", 0) + 166.83394354349454 * max(earth_input."PC1" - 1.9193410513160611, 0) + -178.68275200317646 * max(earth_input."PC4" - 0.1047646025983524, 0) + -1777.9683172986129 * max(earth_input."PC1" - -0.674082655111522, 0) + 1642.202521023471 * max(earth_input."PC1" - -0.8617563505066398, 0) + 10.146013831606671 AS "Score_1", 8.992786820244358 * max(-0.017360589210772742 - earth_input."PC1", 0) + -15.849955447247641 * max(earth_input."PC3" - 0.2280435636734822, 0) + 199.16131724211783 * max(0.2280435636734822 - earth_input."PC3", 0) + -10.365620460304324 * max(earth_input."PC1" - 1.9193410513160611, 0) + 182.7411014213087 * max(earth_input."PC4" - 0.1047646025983524, 0) + 1268.4777055539728 * max(earth_input."PC1" - -0.674082655111522, 0) + -1230.34115471282 * max(earth_input."PC1" - -0.8617563505066398, 0) + -10.146013832189793 AS "Score_2" 
FROM earth_input), 
earth_model_cte_logistic AS 
(SELECT earth_model_cte."KEY" AS "KEY", earth_model_cte."Score_0" AS "Score_0", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -earth_model_cte."Score_0")))) AS "Logistic_0", earth_model_cte."Score_1" AS "Score_1", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -earth_model_cte."Score_1")))) AS "Logistic_1", earth_model_cte."Score_2" AS "Score_2", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -earth_model_cte."Score_2")))) AS "Logistic_2" 
FROM earth_model_cte), 
orig_cte AS 
(SELECT earth_model_cte_logistic."KEY" AS "KEY", earth_model_cte_logistic."Score_0" AS "Score_0", earth_model_cte_logistic."Score_1" AS "Score_1", earth_model_cte_logistic."Score_2" AS "Score_2", earth_model_cte_logistic."Logistic_0" / (earth_model_cte_logistic."Logistic_0" + earth_model_cte_logistic."Logistic_1" + earth_model_cte_logistic."Logistic_2") AS "Proba_0", earth_model_cte_logistic."Logistic_1" / (earth_model_cte_logistic."Logistic_0" + earth_model_cte_logistic."Logistic_1" + earth_model_cte_logistic."Logistic_2") AS "Proba_1", earth_model_cte_logistic."Logistic_2" / (earth_model_cte_logistic."Logistic_0" + earth_model_cte_logistic."Logistic_1" + earth_model_cte_logistic."Logistic_2") AS "Proba_2", CAST(NULL AS FLOAT) AS "LogProba_0", CAST(NULL AS FLOAT) AS "LogProba_1", CAST(NULL AS FLOAT) AS "LogProba_2", CAST(NULL AS BIGINT) AS "Decision", CAST(NULL AS FLOAT) AS "DecisionProba" 
