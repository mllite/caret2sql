...
(SELECT "ADS"."KEY" AS "KEY", CASE WHEN ("ADS"."Feature_9" <= CAST(0.015023609022 AS DOUBLE PRECISION)) THEN 2 ELSE CASE WHEN ("ADS"."Feature_6" <= CAST(0.522071829574 AS DOUBLE PRECISION)) THEN 4 ELSE 5 END END AS node_id_2 
FROM "BinaryClass_10" AS "ADS"), 
dummy_cte AS 
(SELECT CAST(0.0 AS DOUBLE PRECISION) AS "X"), 
"DT_node_data" AS 
(SELECT "Values".nid AS nid, "Values"."P_0" AS "P_0", "Values"."P_1" AS "P_1", "Values"."D" AS "D", "Values"."DP" AS "DP" 
FROM (SELECT 2 AS nid, CAST(0.808510638297872 AS DOUBLE PRECISION) AS "P_0", CAST(0.191489361702128 AS DOUBLE PRECISION) AS "P_1", 0 AS "D", CAST(0.808510638297872 AS DOUBLE PRECISION) AS "DP" 
...
FROM dummy_cte UNION ALL SELECT 5 AS nid, CAST(0.285714285714286 AS DOUBLE PRECISION) AS "P_0", CAST(0.714285714285714 AS DOUBLE PRECISION) AS "P_1", 1 AS "D", CAST(0.714285714285714 AS DOUBLE PRECISION) AS "DP" 
