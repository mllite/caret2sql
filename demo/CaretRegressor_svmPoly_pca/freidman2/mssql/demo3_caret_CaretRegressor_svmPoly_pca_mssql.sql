...
(SELECT [ADS_pre_1_OUT].[KEY] AS [KEY], (CAST([ADS_pre_1_OUT].[PC1] AS FLOAT(53)) - 7.259817746962938e-17) / 1.0912921970850795 AS [PC1], (CAST([ADS_pre_1_OUT].[PC2] AS FLOAT(53)) - 4.6837533851373785e-17) / 1.0787025043586622 AS [PC2], (CAST([ADS_pre_1_OUT].[PC3] AS FLOAT(53)) - -6.609296443471634e-17) / 0.9655534556992196 AS [PC3], (CAST([ADS_pre_1_OUT].[PC4] AS FLOAT(53)) - -2.203098814490545e-17) / 0.8445050454904731 AS [PC4] 
FROM [ADS_pre_1_OUT]), 
[SV_data] AS 
...
FROM (SELECT full_join_data_sv.[KEY] AS [KEY], sum(CAST(full_join_data_sv.dot_prod1 AS FLOAT(53))) + 0.0303245372537232 AS dot_product 
FROM (SELECT kernel_input_with_scaling.[KEY] AS [KEY], [SV_data].dual_coeff * power(CAST(0.1 * (kernel_input_with_scaling.[PC1] * [SV_data].sv_0 + kernel_input_with_scaling.[PC2] * [SV_data].sv_1 + kernel_input_with_scaling.[PC3] * [SV_data].sv_2 + kernel_input_with_scaling.[PC4] * [SV_data].sv_3) + 1.0 AS FLOAT(53)), 3.0) AS dot_prod1 
