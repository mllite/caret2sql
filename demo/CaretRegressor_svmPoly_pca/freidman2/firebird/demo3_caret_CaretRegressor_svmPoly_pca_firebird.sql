...
FROM (SELECT 0 AS sv_idx, -0.253383876033811 AS dual_coeff, 1.37850089854269 AS sv_0, 1.191161406549864 AS sv_1, 0.05089855230848973 AS sv_2, -0.8964391596224827 AS sv_3 FROM rdb$database UNION ALL SELECT 1 AS sv_idx, 0.01855903373919684 AS dual_coeff, 0.15237495262279765 AS sv_0, -0.7546459295194593 AS sv_1, 1.8534269105418688 AS sv_2, -1.1168298691466336 AS sv_3 FROM rdb$database UNION ALL SELECT 2 AS sv_idx, 0.2784388218690412 AS dual_coeff, -0.5771110361354514 AS sv_0, 0.5484588455313958 AS sv_1, 1.3160792801997816 AS sv_2, 1.3039486790441286 AS sv_3 FROM rdb$database UNION ALL SELECT 3 AS sv_idx, 0.007962502075542329 AS dual_coeff, -0.11504934855312395 AS sv_0, -0.5417883638361547 AS sv_1, -1.941763685449151 AS sv_2, -1.701579213947659 AS sv_3 FROM rdb$database UNION ALL SELECT 4 AS sv_idx, -0.13201583387729118 AS dual_coeff, -1.3484161330190985 AS sv_0, 0.3619825146745239 AS sv_1, 0.20177401272176335 AS sv_2, 2.3281536354101884 AS sv_3 FROM rdb$database UNION ALL SELECT 5 AS sv_idx, -0.07422423748355764 AS dual_coeff, -0.33139320711377185 AS sv_0, -1.8324326878696413 AS sv_1, 0.615337245382805 AS sv_2, 1.730019845417768 AS sv_3 FROM rdb$database UNION ALL SELECT 6 AS sv_idx, 0.5415093334215412 AS dual_coeff, 1.1623381422247647 AS sv_0, -0.2946826758671762 AS sv_1, 1.5730523524737925 AS sv_2, 0.6221839714024429 AS sv_3 FROM rdb$database UNION ALL SELECT 7 AS sv_idx, -0.023257890980353746 AS dual_coeff, -0.4166382298158355 AS sv_0, 2.378813663050534 AS sv_1, -0.902583443484049 AS sv_2, -0.868927903554924 AS sv_3 FROM rdb$database UNION ALL SELECT 8 AS sv_idx, 0.2527075890284157 AS dual_coeff, -0.16828540326040994 AS sv_0, -1.2518904350496618 AS sv_1, 2.0197246545043206 AS sv_2, -0.8039021154910663 AS sv_3 FROM rdb$database UNION ALL SELECT 9 AS sv_idx, -0.2335490355201043 AS dual_coeff, -1.4925296439214475 AS sv_0, -1.2101666301463625 AS sv_1, -0.13193963626985628 AS sv_2, -0.2283141200818075 AS sv_3 FROM rdb$database UNION ALL SELECT 10 AS sv_idx, -0.04266903498006475 AS dual_coeff, -0.5650957902827647 AS sv_0, -1.8692319578289327 AS sv_1, -0.6984537411456064 AS sv_2, 0.9435761306003548 AS sv_3 FROM rdb$database UNION ALL SELECT 11 AS sv_idx, -0.09537706727258828 AS dual_coeff, 2.339879525546283 AS sv_0, 1.1714521819539243 AS sv_1, -0.12302295652274545 AS sv_2, 0.5224606031209847 AS sv_3 FROM rdb$database UNION ALL SELECT 12 AS sv_idx, -0.4737849563387233 AS dual_coeff, 1.990921411400796 AS sv_0, 0.1383924046659286 AS sv_1, -0.31028705038580057 AS sv_2, -0.4961920992645837 AS sv_3 FROM rdb$database UNION ALL SELECT 13 AS sv_idx, 0.15991084736448266 AS dual_coeff, -0.07615280303272615 AS sv_0, 1.052700600607052 AS sv_1, 1.0934129854728014 AS sv_2, 0.7220233633050731 AS sv_3 FROM rdb$database UNION ALL SELECT 14 AS sv_idx, -0.4153331337683592 AS dual_coeff, -2.022673441251131 AS sv_0, -0.1084335030849492 AS sv_1, -0.12626174946808813 AS sv_2, 0.6100438112889778 AS sv_3 FROM rdb$database UNION ALL SELECT 15 AS sv_idx, 0.4845069387566333 AS dual_coeff, -1.171639821886214 AS sv_0, 0.5442159347926955 AS sv_1, 1.684051478875226 AS sv_2, -0.6066278790061805 AS sv_3 FROM rdb$database) AS "Values"), 
kernel_dp AS 
(SELECT t."KEY" AS "KEY", t.dot_product AS dot_product 
FROM (SELECT full_join_data_sv."KEY" AS "KEY", sum(CAST(full_join_data_sv.dot_prod1 AS DOUBLE PRECISION)) + 0.0303245372537232 AS dot_product 
