...
(SELECT "ADS_pre_1_OUT"."KEY" AS "KEY", (CAST("ADS_pre_1_OUT"."PC1" AS FLOAT) - 7.259817746962938e-17) / 1.0912921970850795 AS "PC1", (CAST("ADS_pre_1_OUT"."PC2" AS FLOAT) - 4.6837533851373785e-17) / 1.0787025043586622 AS "PC2", (CAST("ADS_pre_1_OUT"."PC3" AS FLOAT) - -6.609296443471634e-17) / 0.9655534556992196 AS "PC3", (CAST("ADS_pre_1_OUT"."PC4" AS FLOAT) - -2.203098814490545e-17) / 0.8445050454904731 AS "PC4" 
FROM "ADS_pre_1_OUT"), 
"SV_data" AS 
...
FROM (SELECT full_join_data_sv."KEY" AS "KEY", sum(CAST(full_join_data_sv.dot_prod1 AS FLOAT)) + 0.0303245372537232 AS dot_product 
