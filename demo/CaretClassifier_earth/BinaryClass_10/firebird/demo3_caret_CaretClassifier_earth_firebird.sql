...
earth_cte AS 
(SELECT earth_input."KEY" AS "KEY", -2.6730128206194 * maxvalue(1.14281924773009 - earth_input."Feature_9", 0) + 3.7137130158392786 AS earth 
FROM earth_input), 
dot_prod_logistic AS 
(SELECT earth_cte."KEY" AS "KEY", earth_cte.earth AS dot_prod, 1.0 / (1.0 + exp(CASE WHEN (minvalue(100.0, maxvalue(-100.0, -earth_cte.earth)) >= -709.782712893384) THEN minvalue(100.0, maxvalue(-100.0, -earth_cte.earth)) ELSE -709.782712893384 END)) AS logistic 
FROM earth_cte)
