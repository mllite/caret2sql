...
earth_cte AS 
(SELECT earth_input."KEY" AS "KEY", -2.6730128206194 * max(1.14281924773009 - earth_input."Feature_9", 0) + 3.7137130158392786 AS earth 
FROM earth_input), 
dot_prod_logistic AS 
(SELECT earth_cte."KEY" AS "KEY", earth_cte.earth AS dot_prod, 1.0 / (1.0 + exp(min(100.0, max(-100.0, -earth_cte.earth)))) AS logistic 
FROM earth_cte)
